...
async def call_codex_naturalness(novel_dir: str, episode_file: str) -> str:
    ...
    ep_path = episode_file
    today = datetime.now().strftime("%Y-%m-%d %H:%M")
    prompt = f"""오늘 날짜는 {today}이다. {ep_path}를 읽고 한국어 결합 자연성을 검사해서 EDITOR_FEEDBACK_gpt_naturalness.md에 결과를 작성해.

너는 한국어 원어민 감각으로 모든 문장을 읽는다. 각 문장에서 명사-동사, 부사-동사, 감정-신체, 추상명사-서술어의 결합이 한국어 화자가 실제로 쓰는 조합인지 확인한다.

핵심: 문법은 맞지만 "아무도 이렇게는 안 쓴다"에 해당하는 표현을 찾아라. 한국어 화자가 같은 의미를 말할 때 보통 다른 동사/다른 결합을 택하는 문장이 대상이다.

판정 질문:
1. 이 표현을 한국어 화자가 실제로 쓸 가능성이 높은가?
2. 같은 뜻에서 더 관용적인 결합이 따로 있는가?
3. 비슷한 자연스러운 표현에서 단어만 바꾼 것처럼 보이는가?

예외: 시적 비유, 장르적 표현, 캐릭터 고유 어법은 허용.

출력 (EDITOR_FEEDBACK_gpt_naturalness.md에 작성):
# 결합 자연성 검사

| # | 위치(줄) | 원문 | 왜 어색한가 | 자연한 대안 |
|---|---------|------|-----------|-----------|

진짜 어색한 것만 지적하고, 문학적 허용은 보호해라. 결함 없으면 "결함 없음"."""
